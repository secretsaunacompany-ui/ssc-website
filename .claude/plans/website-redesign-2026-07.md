...
| ~~Q14~~ | ~~Formspree plan tier~~ — **RESOLVED by investigation, no longer gating.** Autoresponse is Professional/Business-only, dashboard-configured, and needs an `email` field the form already has. The success panel now branches on a `site.json` flag instead of waiting on the answer, so it is true whether or not the feature is on. | — |
| **Q15** | **WiFi controller — ANSWERED 2026-07-28: add at $2,000** as an individually-selectable line. Makes the Premium Package's savings claim true ($8,000 basket vs $7,000 package) instead of backwards. Ted adds it to `modals/sauna.njk` and the total logic. | WP-0c |
| Q8 | Formspree auto-reply — enable? (recommended) | **WP-0b** |
| Q9 | Laser cut file — does a real one exist? | WP-4 |
| ~~Q10~~ | ~~Badge removal from nav~~ — **ANSWERED 2026-07-28: remove it.** Saul's spec proceeds — 22px wordmark in nav, badge to the footer as a maker's seal at 72px, redrawn from `~/marvin/content/assets/logo-original.pdf`. **WP-2a unblocked.** | — |
