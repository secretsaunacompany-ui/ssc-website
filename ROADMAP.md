...
next: "Wave B candidates, all queued in the plan: mobile-gutter batch + ember-dark decision; modal-scoping batch (carries the dom-integrity cap raise); WP-2 composition / WP-3 copy / WP-4 pages (unblocked by Lee's remaining fact answers); case studies (parked pending client agreements); npm-audit remediation; ssc-ops tracker root-cause fix (cross-repo)."
deploy_gates:
