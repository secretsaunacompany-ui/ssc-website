module.exports = function(eleventyConfig) {
  ...
  /**
   * Sub-page hero responsive rungs (doc 10 s2.3 / s5.3).
   *
   * `src/_data/heroVariants.json` is generated by scripts/image-variants.mjs and
   * records, per stem, the widths that ACTUALLY EXIST in src/img plus the pixel
   * dimensions of the widest declared rung. These two filters are the only way a
   * template reads it, and they FAIL CLOSED on an unknown stem, exactly like the
   * `jsonld` filter above.
   *
   * That failing-closed is the whole point and it is not theoretical. A bare
   * dictionary lookup on a missing stem returns undefined, the loop over it
   * emits `srcset=""`, and the page builds, renders, and ships with no srcset,
   * no browser error, and nothing for an audit to catch. The header of
   * scripts/cloudinary-responsive.test.mjs records this repo being burned by
   * that exact silent-empty shape once already. A template naming a stem that
   * has no rungs is a broken template, so the build stops.
   */
  const heroVariants = require("./src/_data/heroVariants.json");

  const heroVariantEntry = (stem, filterName) => {
    if (typeof stem !== "string" || stem === "") {
      throw new Error(`${filterName}: expected a non-empty stem string, got `
        + `${typeof stem} (${JSON.stringify(stem)}). A hero image field is missing from the `
        + `page's front matter.`);
    }
    const entry = heroVariants[stem];
    if (!entry) {
      throw new Error(`${filterName}: "${stem}" is not in src/_data/heroVariants.json. `
        + `Add the stem to HERO_STEMS in scripts/image-variants.mjs and re-run it, so the `
        + `rungs on disk and the rungs the srcset names cannot disagree.`);
    }
    return entry;
  };

  // The widths on disk for a stem, ascending. Consumed by the srcset loop and
  ...
  // have, which is the defect rhythm.test.mjs's B2 check exists to catch.
  eleventyConfig.addFilter("heroDims", (stem) => {
    const { w, h } = heroVariantEntry(stem, "heroDims");
    return { w, h };
  });
